...
if(isset($_GET['elemid'])) {
    ...
    switch($elemeid) {
// affichier les nombres de chaques risques lors de la selection d'un quartier
      case 'information_quartier':


       // Valide et nettoie l'entrée pour prévenir l'injection SQL
                $id_quartier = filter_var($_GET['id_quartier'], FILTER_VALIDATE_INT);

                if ($id_quartier === false) {
                    http_response_code(400); // Requête incorrecte
                     header('Content-Type: application/json'); // Assurez-vous que l'en-tête est là
                    ob_clean();
                    echo json_encode(['error' => 'ID de quartier invalide.']);
                    exit();
                }
...
COUNT(*) AS tt_risques
FROM informations 
WHERE quartier = :id_quartier'; // Utilise un placeholder where commune = ' . $id_commune;
...
$stmt = $conn->prepare($sql);
    ...
    if ($id_quartier === false) {
        ...
        header('Content-Type: application/json');
        ob_clean();
        echo json_encode(['error' => 'ID de quartier invalide pour les risques.']);
        exit();
    }
    ...
    $sql = 'SELECT
                ...
                risques,
                description,
                fichier, -- Assurez-vous que cette colonne est sélectionnée
                -- public.ST_AsGeoJSON(public.ST_Transform(geom, 4326)) as geom_geojson
                latitude,   -- Assurez-vous que ces colonnes existent
                longitude   -- Assurez-vous que ces colonnes existent
            FROM informations
            WHERE quartier= :id_quartier';
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $risk = [
            ...
            'fichier' => $row['fichier'], // C'est ici que le nom du fichier est récupéré
            'latitude' => (float)$row['latitude'],  // Convertir en float
            'longitude' => (float)$row['longitude'] // Convertir en float
        ];
        $risks_data[] = $risk;
    }
    ...
    break; 
        ...
        case 'zoom_quartier':
                ...
                $sql = 'SELECT id, public.ST_AsGeoJSON(ST_Transform(geom, 4326)) as geom, nom_quar as nom_quartier
            FROM public.quartiers_cocody
    ...
    $feature = [
        ...
        'properties' => [ // Créez directement un tableau avec les propriétés que vous voulez
            ...
            'nom_quartier' => $row['nom_quartier'] // C'est cette ligne qui est cruciale
        ]
    ];
 ...
 // <<< NOUVELLE CASE : Récupérer tous les risques pour l'affichage initial
            case 'get_all_risks':
                $sql = 'SELECT
                            ...
                            latitude,
                            longitude
                        FROM informations'; // Pas de clause WHERE pour récupérer tous les risques
                        
                $stmt = $conn->prepare($sql);
                // Pas de bindParam car pas de paramètre de commune
                $stmt->execute();

                $all_risks_data = [];
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    $risk = [
                        'id' => (int)$row['id_information'],
                        'risques' => $row['risques'],
                        'description' => $row['description'],
                        'fichier' => $row['fichier'],
