...
switch ($_GET['elemid']) {
    ...
    case 'get_risks_by_quartier':
        ...
        if ($selectedCommune !== 'Abidjan' && $selectedQuartier !== 'Tous') {
            $query_quartiers = " AND quartier = " . $conn->quote($selectedQuartier);
        } elseif ($selectedCommune !== 'Abidjan') {
            // Pour filtrer les quartiers d'une commune, on doit joindre la table informations
            // avec la table quartiers_abidjan.
            // Cependant, la table 'informations' n'a pas la colonne 'commune'.
            // On peut s'en tenir au filtrage par quartier pour le moment.
            // Pour un filtrage correct, il faudrait ajouter une colonne 'commune'
            // dans la table 'informations' lors de l'insertion des données.
        }
