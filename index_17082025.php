...
// $sql = 'SELECT id, "commune" as nom_commune  FROM public.communes_abidjan';
 $sql = 'SELECT id, "nom_quar" as nom_quartier  FROM public.quartiers_cocody';
...
while($row = $result->fetch(PDO::FETCH_ASSOC)){
  ...
  // $liste_commune.= '<li><a href="#" onclick="zoomCommune(\''.$row['id'].'\');">'.$row['nom_commune'].'</a></li>';
    $liste_quartier.= '<li><a href="#" onclick="zoomQuatier(\''.$row['id'].'\');">'.$row['nom_quartier'].'</a></li>';
	}  
...
.container {
  ...
  flex-direction: row;
  height: 100vh;
  width: 100vw; /* Assure que le conteneur prend toute la largeur de la fenêtre */
  overflow: hidden;
    ...
    <section class="sidebar">
         ...
         <!-- search form -->
          ...
          <!-- sidebar menu: : sty
           ...
           le can be found in sidebar.less -->
          ...
          <li class="treeview">
            ...
            <li class="treeview">
             ...
             <li class="treeview">
              <a href="analyste/index.php"><i class="fa fa-"></i><span>Analystes</span></a>
            </li>
             <li class="treeview">
...
<h3>Bienvenue dans l'application d'alerte à l'inondation</h3>

<p> Cette application web cartographique a été conçue pour renforcer la sécurité des citoyens et 
accompagner les autorités dans la gestion des risques liés aux inondations. </br> 
 Grâce à des données reuillies en temps réel, une d’alerte est envoyée aux citoyens par mail et sms. </br> 
 Cette alerte vous informe en amont pour mieux vous protéger. </p> 
 </div>
...
// caracteristique de la couche quartier
//var limite_commune = L.geoJson(null,{
          ...
          var limite_quartier = L.geoJson(null,{
				style: function(feature){
				return {
					color: 'blue', // Couleur pour la quartier zoomée
					weight: 2,
					fill: true,
					fillColor: 'lightblue',
					fillOpacity: 0.6
...
// Couche pour toutes les communes (chargée depuis le fichier JSON statique)
  ...
  var quartiers_cocody = L.geoJson(null,{
						style: function(feature){
							return {
								color: 'black',
								weight: 2,
          ...
          //  'Limite abidjan' : communes_abidjan, // C'est ici que votre couche principale sera listée
						'Limite cocody' : quartiers_cocody,
...
// Fonction pour récupérer les informations du quartier (statistiques)
      function headerQuartier(id_quartier) {
        var url = 'ajax.php?elemid=information_quartier&id_quartier='+id_quartier;
        var xhr = getXhr();
        xhr.onreadystatechange = function() {
        if(xhr.readyState == 4) {
          // IMPORTANT : Vérifiez la réponse du serveur
            console.log('Réponse brute de information_quartier :', xhr.responseText);
            try {
...
var retourJson = JSON.parse(xhr.responseText);
                if (retourJson.error) {
                    ...
                    alert('Infos quartier : ' + retourJson.tt_inondation + ' inondations, ' + retourJson.tt_erosion + ' érosions, etc.');
                }
            } catch (e) {
                console.error('Erreur de parsing JSON pour information_quartier:', e);
                alert('Erreur de communication avec le serveur pour les informations du quartier.');
            }
        }
    }
...
//Ajouter les couches
  ...
  // <<< MODIFICATION ICI : Efface toutes les couches de risque catégorisées
   ...
   var url_zoom = 'ajax.php?elemid=zoom_quartier&id_quartier=' + id_quartier;
    var xhr_zoom = getXhr();
...
xhr_zoom.onreadystatechange = function() {
 ...
 // 2. Vérifier la réponse brute du serveur
           console.log('Réponse brute de zoom_quartier pour ID ' + id_quartier + ':', xhr_zoom.responseText);
        try {
            var json_quartier = JSON.parse(xhr_zoom.responseText);

                if (json_quartier.error) {
                    console.error('Erreur AJAX signalée par PHP (zoom):', json_quartier.error);
                    alert('Erreur lors du zoom: ' + json_quartier.error);
                ...
                // 3. Vérifier si des fonctionnalités GeoJSON sont présentes
                 ...
                 // --- NOUVEAU CODE À AJOUTER ---
                   if (json_quartier && json_quartier.features && json_quartier.features.length > 0) {
                    var firstFeature = json_quartier.features[0];
                    if (!firstFeature.geometry || firstFeature.geometry.coordinates.length === 0) {
                        console.warn('La géométrie de la première fonctionnalité est vide ou manquante pour ID ' + id_quartier);
                        // alert('La géométrie du quartier est vide ou invalide.'); // Laissez ceci commenté
                        return;
                    }
                    // --- FIN NOUVEAU CODE ---
                  ...
                  // 4. Ajouter les données à la couche `limite_quartier`
                    limite_quartier.addData(json_quartier);

                    // 5. Ajuster le zoom de la carte aux limites de la géométrie reçue
                    // Vérifiez que limite_quartier a bien des limites après l'ajout des données
                    if (limite_quartier.getBounds().isValid()) {
                        carte.fitBounds(limite_quartier.getBounds());
                        console.log('Zoom effectué sur la quartier ID:', id_quartier);
      ...
      var url_stats = 'ajax.php?elemid=information_quartier&id_quartier=' + id_quartier;
      var xhr_stats = getXhr();

      xhr_stats.onreadystatechange = function() {
        if (xhr_stats.readyState == 4) {
        console.log('Réponse brute des stats pour ID ' + id_quartier + ':', xhr_stats.responseText);
          try {
            var stats = JSON.parse(xhr_stats.responseText);
              ...
              if (stats.error) {
               ...
               // <<< NOUVELLE SECTION : Appel AJAX pour les coordonnées des risques et les popups
                var url_risks_coords = 'ajax.php?elemid=get_risks_coordinates&id_quartier=' + id_quartier;
                var xhr_risks_coords = getXhr();

                  xhr_risks_coords.onreadystatechange = function() {
                    if (xhr_risks_coords.readyState === 4) {
                     console.log('Réponse brute (coordonnées risques) pour ID ' + id_quartier + ':', xhr_risks_coords.responseText);
                    try {
...
//  si c'est GeoJSON ou lat/lon
 ...
 // >>> FIN NOUVELLE SECTION
              ...
              document.getElementById('nombre_risque').textContent = 'Erreur';
                        ...
                        console.warn('La couche limite_quartier n\'a pas de limites valides pour zoomer, même après l\'ajout de données.');
                        alert('Impossible de zoomer sur ce quartier : géométrie non valide pour le zoom.');
                        }
                     // }
                } else {
                     console.warn('Aucune donnée GeoJSON valide ou aucune fonctionnalité trouvée pour le quartier ID ' + id_quartier);
                    alert('Impossible de trouver la géométrie pour ce quartier.');
                }
            } catch (e) {
                console.error('Erreur de parsing JSON pour zoom_quartier:', e);
                alert('Erreur lors du traitement des données de la quartier. Vérifiez la console.');
            }
        }
    };
...
$(document).ready(function() {
    ...
    $.getJSON("data/quartiers_cocody.json", function(donnee) {
        quartiers_cocody.addData(donnee);
        quartiers_cocody.addTo(carte); // Ajoute la couche de tous les quartiers à la carte

        // Ajuste le zoom initial de la carte pour inclure toutes les quartiers chargées
        if (quartiers_cocody.getBounds().isValid()) {
            carte.fitBounds(quartiers_cocody.getBounds());
            console.log('Zoom initial sur tous les quartiers effectué.');
        } else {
            console.warn('La couche quartiers_cocody n\'a pas de géométrie valide pour le zoom initial.');
        alert('Impossible de zoomer, la géométrie retournée est dégénérée ou trop petite.');
          }
    }).fail(function(jqxhr, textStatus, error) {
        var err = textStatus + ", " + error;
        console.error("Erreur lors du chargement de quartiers_cocody.json: " + err);
    });
